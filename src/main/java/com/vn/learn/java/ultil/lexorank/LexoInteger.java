package com.vn.learn.java.ultil.lexorank;

import com.vn.learn.java.ultil.lexorank.system.LexoNumeralSystem;
...
public class LexoInteger implements Comparable<LexoInteger> {
  ...
  public static LexoInteger parse(String strFull, LexoNumeralSystem system) {
    ...
    for (int magIndex = 0; strIndex >= 0; ++magIndex, --strIndex) {
  ...
  public LexoInteger subtract(LexoInteger other) {
    ...
    return cmp < 0
        ? make(sys, getSign(sign == -1, 1, -1), subtract(sys, other.mag, mag))
        : make(sys, getSign(sign == -1, -1, 1), subtract(sys, mag, other.mag));
  }

  private int getSign(boolean sign, int x, int x1) {
    return sign ? x : x1;
  }
  ...
  public LexoInteger negate() {
    return isZero() ? this : make(sys, getSign(sign == 1, -1, 1), mag);
  }
  ...
  @Override
  public int hashCode() {
    int result = Objects.hash(NEGATIVE_SIGN, ZERO_SIGN, POSITIVE_SIGN, sign, sys);
  ...
  public int compareTo(LexoInteger lexoInteger) {
    ...
    if (sign == -1) {
      return compareWhenSignNegative(lexoInteger);
    }
  ...
  private int compareWhenSignNegative(LexoInteger lexoInteger) {
